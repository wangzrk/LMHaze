...
#* change to tripple folder
def tripple_paths_from_folder(folders, keys, filename_tmpl):
    """Generate tripple paths from folders.
    ...
    assert len(folders) == 3, (
        'The len of folders should be 3 with [input_folder, gt_folder, logit_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 3 with [input_key, gt_key, logit_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder, logit_folder = folders
    input_key, gt_key, logit_key = keys

    input_paths = list(scandir(input_folder))
    gt_paths = list(scandir(gt_folder))
    logit_paths = list(scandir(logit_folder))

    assert len(input_paths) == len(gt_paths), (
        f'{input_key} and {gt_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(gt_paths)}.')
    assert len(input_paths) == len(logit_paths), (
        f'{input_key} and {logit_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(logit_paths)}.')
    ...
    for idx in range(len(gt_paths)):
        ...
        logit_path = logit_paths[idx]
        basename_logit, ext_logit = osp.splitext(osp.basename(logit_path))
        ...
        logit_path = osp.join(logit_folder, logit_path)

        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path),
                  (f'{logit_key}_path', logit_path)]))
    return paths

def paired_paths_from_folder_logit(folders, keys, filename_tmpl):
    ...
    assert len(keys) == 2, (
        'The len of keys should be 2 with [input_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, logit_folder = folders
    input_key, logit_key = keys
    ...
    assert len(input_paths) == len(logit_paths), (
        ...
        f'{len(input_paths)}, {len(logit_paths)}.')
    paths = []
    for idx in range(len(logit_paths)):
        logit_path = logit_paths[idx]
        basename, ext = osp.splitext(osp.basename(logit_path))
        input_path = input_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(input_path))
        input_name = f'{filename_tmpl.format(basename)}{ext_input}'
        input_path = osp.join(input_folder, input_name)
