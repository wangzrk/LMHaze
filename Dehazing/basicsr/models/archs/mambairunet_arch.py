...
class SSM_Block(nn.Module):
    ...
    # @torchsnooper.snoop()
    def forward(self, x: torch.Tensor, B, H, W):   # x: (2, 384, 64, 64)
...
class VSSM_moe(nn.Module):
    def __init__(
        ...
        self.dropout = nn.Dropout(dropout) if dropout > 0. else None
        #* set experts number
        self.moe_blocks = nn.ModuleList([SSM_Block(d_model=d_model) for i in range(14)])
        # self.ssm_block = nn.ModuleList([SSM_Block(d_model=d_model)])
        # print(f'Initialized moe_blocks with {len(self.moe_blocks)} SSM_Block instances')
        # self.logit_proj = nn.Linear(4096, self.d_inner, bias=bias, **factory_kwargs)                    # just for ablation
        # self.logit_cat_proj = nn.Linear(self.d_inner * 2, self.d_inner, bias=bias, **factory_kwargs)    # just for ablation
    ...
    # @torchsnooper.snoop()
    def forward(self, x: torch.Tensor, logit, **kwargs):

        # print(f'In forward pass, moe_blocks contains {len(self.moe_blocks)} SSM_Block instances')
        ...
        logit_32 = logit.to(torch.float32)

        topk_params = [3, 2, 1, 1]
        part_indices = [(0, 5), (5, 8), (8, 11), (11, 14)]    

        outputs = []
        all_topk_indices = []

        for (start, end), topk in zip(part_indices, topk_params):

            part_logits = logit_32[:, start:end]
            _, topk_indices = part_logits.topk(topk, dim=1)
            
            part_topk_indices = topk_indices + start  
            all_topk_indices.append(part_topk_indices)
            
            # mixture of experts
            for i in range(topk):
                block_idx = part_topk_indices[0][i]
                expert_block = self.moe_blocks[block_idx]
                outputs.append(expert_block(x, B, H, W))
...
class VSSBlock_moe(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention =  VSSM_moe(d_model=hidden_dim,expand=expand,dropout=attn_drop_rate, **kwargs)
        self.drop_path = DropPath(drop_path)
        self.skip_scale= nn.Parameter(torch.ones(hidden_dim))
        self.conv_blk = CAB(hidden_dim)
        self.ln_2 = nn.LayerNorm(hidden_dim)
    ...
    # @torchsnooper.snoop()
    def forward(self, input, logit, x_size):   
        # x [B,HW,C]
        B, L, C = input.shape                           
        input = input.view(B, *x_size, C).contiguous()  
        x = self.ln_1(input)                            
        moe_logit = logit
        x = input*self.skip_scale + self.drop_path(self.self_attention(x, moe_logit))
        x = x*self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        x = x.view(B, -1, C).contiguous()               
        return x
##########################################################################
...
class MambaIRUNet(nn.Module):
    def __init__(self,
        ...
        base_d_state = 4
        # add moe block
        self.encoder_level1_moe = nn.ModuleList([
            VSSBlock_moe(
                hidden_dim=dim,
        ...
        # add moe blocks
        self.decoder_level1_moe = nn.ModuleList([
            VSSBlock_moe(
                hidden_dim=int(dim * 2 ** 1),
                drop_path=drop_path_rate,
                norm_layer=nn.LayerNorm,
    ...
    def forward(self, inp_img, logit):
        ...
        for layer in self.encoder_level1_moe:    
            out_enc_level1 = layer(out_enc_level1, moe_logit, [H, W])
        ...
        for layer in self.decoder_level1_moe:
            out_dec_level1 = layer(out_dec_level1, moe_logit, [H, W])
